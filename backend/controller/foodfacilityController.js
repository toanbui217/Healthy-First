...
router.post("/insert", (req, res) => {
  ...
  foodfacility
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message + ", Error when create foodfacilityData.",
      });
    });
});
...
router.post("/insertmultiple", (req, res) => {
  ...
  // forLoop();
  res.send({
    message: "successfully added " + foodfacilityList.length + " students",
  });
...
router.put("/extenddate/:id", authRole(ROLE.ADMIN), (req, res) => {
  ...
  Certification.findOneAndUpdate(
    ...
    .then((data) => {
      if (!data)
        res.status(404).send({
          message: "Not found student with id " + certificationID,
        });
      else {
        res.status(200).send(data);
      }
    ...
    .catch((err) => {
      res.status(500).send({
        message: "Error retrieving student with id=" + certificationID,
      });
    });
});
...
function setFoodFacility(req, res, next) {
  ...
  req.foodfacility = FoodFacility.find(
    (foodfacility) => foodfacility.location === foodfacilityLocation
  );
  ...
  FoodFacility.find({ "address.district": district })
    ...
    .then((data) => {
      ...
      if (req.foodfacility.length == 0) {
        console.log("check");
        res.status(404);
        return res.send("FoodFacility not found");
      }
      next();
    });
}
